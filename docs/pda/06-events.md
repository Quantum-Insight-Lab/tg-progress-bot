...
5. **У события есть инварианты, атомы и проекции.** `invariants` — INV из [04](04-invariants.md), `realizes` — атомы ТЗ, которые событие реализует, `projections` — проекции из [07](07-architecture.md), которые оно перерисовывает. `tz-check` проверяет, что INV и проекции существуют, атомы живы, а каждое событие из таблицы актов ([02](02-certainty-acts.md)) есть в реестре.
