...
5. **У события есть инварианты и атомы.** `invariants` — INV из [04](04-invariants.md), `realizes` — атомы ТЗ, которые событие реализует. `tz-check` проверяет, что INV существуют, атомы живы, а каждое событие из таблицы актов ([02](02-certainty-acts.md)) есть в реестре.
...
Поле `projections` появится в реестре вместе с проекциями (шаг 6б): какие экраны и строки перерисовывает событие.
